...
void daniel::USART::SendMessage( uint8_t const * const pDat , uint16_t const length ) const
{
	...
	constexpr uint16_t const bufSize = 1024 ;

	uint8_t  buf[ bufSize ] ;
	uint16_t len ;

	Stuffing( buf , len , bufSize , pDat , length ) ;
	...
	uint8_t const * const psBuf , uint16_t const & scnt ) const
{
	...
	if( 2 <= scnt && 0x7e == psBuf[ 0 ] && 0x7f == psBuf[ scnt - 1 ] )
	{
